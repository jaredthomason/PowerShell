...
$ouToSearch = "faa.gov/Programs/National/Data_Centers/Servers/WSA_Servers/Application"
#===================================================================================================
...
$script:logfile = "C:\Scripts\POWERSHELL\ComputerDetails\logs\ComputerDetailsWSAApplication-$(get-date -format MM-dd-yyyy-HH-mm-ss).txt"
$script:separator = @"
$("-" * 25)
"@
...
function Get-OSWMI ([string]$ComputerName) {
	...
	$strOS = ""
	if ($computername -ne "JLAXRAPNSV01RF") {
...
function Create-Spreadsheet ($colComputerDetails) {
	...
	$Workbook.SaveAs("$ExcelFileDirectory\ComputerDetails-WSAApplicaiton.xls")
	$Excel.quit()
